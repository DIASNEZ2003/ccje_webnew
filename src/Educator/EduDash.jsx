...
// Import your newly created components here!
    ...
    <>
      ...
      <div className="flex h-screen w-screen overflow-hidden bg-[#F8F9FA] font-['Inter',sans-serif]">
        ...
        <aside className={`relative z-40 flex h-full flex-col bg-white border-r border-gray-200 shadow-[4px_0_24px_rgba(0,0,0,0.02)] transition-all duration-300 ease-in-out ${sidebarCollapsed ? 'w-20' : 'w-72'}`}>
          ...
          <div className={`flex flex-col items-center border-b border-gray-100 py-4 bg-gradient-to-b from-white to-gray-50/50 ${sidebarCollapsed ? 'px-2' : 'px-6'}`}>
            <div className="relative group">
              <button 
                ...
                style={{ width: sidebarCollapsed ? '40px' : '60px', height: sidebarCollapsed ? '40px' : '60px' }}
                ...
                {uploading ? (
                  ...
                  <>
                    ...
                    <div className="absolute inset-0 hidden items-center justify-center bg-black/40 transition-all group-hover:flex">
                      <Camera className="text-white" style={{ width: sidebarCollapsed ? '14px' : '20px', height: sidebarCollapsed ? '14px' : '20px' }} />
                    </div>
                  </>
                ) : (
                  <>
                    <User className="text-gray-400" style={{ width: sidebarCollapsed ? '20px' : '30px', height: sidebarCollapsed ? '20px' : '30px' }} />
                    <div className="absolute inset-0 hidden items-center justify-center bg-black/5 transition-all group-hover:flex">
                      <Camera className="text-gray-600" style={{ width: sidebarCollapsed ? '14px' : '20px', height: sidebarCollapsed ? '14px' : '20px' }} />
                    </div>
                  </>
                )}
              </button>
            ...
            {!sidebarCollapsed && (
              <div className="mt-2 flex w-full flex-col items-center overflow-hidden text-center">
                ...
                <span className="mt-0.5 flex items-center gap-1 w-full justify-center truncate text-[10px] font-semibold uppercase tracking-wider text-[#800000]">
                  Educator
                </span>
              </div>
            )}
          ...
          {/* Sidebar Middle: Navigation Links (Non-scrollable, condensed) */}
              ...
              {navItems.map((item) => {
                ...
                return (
                  ...
                  >
                    ...
                    {!sidebarCollapsed && (
                      <span className="ml-2.5 text-left whitespace-normal leading-snug pr-3">
                        {item.label}
                      </span>
                    )}
                  </button>
        ...
        <main className="flex h-full flex-1 flex-col overflow-hidden">
          ...
          {/* Blank Scrollable Content Container */}
          <div className="flex-1 overflow-y-auto p-8">
            <div className="mx-auto w-full max-w-7xl">
              
              {/* Insert Educator Components Here based on activeTab state */}
              {activeTab === 'dashboard' && <></>}
